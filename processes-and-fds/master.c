...
static const char* MECHANISM_STRINGS[] = { "sequential", "select",
                                           "poll",       "epoll" };
static const int NUM_MECHANISMS = 4;
static const char* USAGE_MESSAGE =
    "Usage: ./master [--worker_path path] [--num_workers num] "
    "[--wait_mechanism "
    "sequential | select | poll | epoll] [-x num] [-n num]";
static const int MAX_WORKERS = 75; // we have a maximum number of workers
                                   // because of the limit of number of open
...
const Arg* parse_arguments(int argc, char** argv) {
  ...
  for (i = 1; i < argc; i += 2) {
    ...
    if (strcmp(argo, ARG_WORKER_PATH) == 0 && a->worker_path == NULL) {
      ...
      *(a->num_workers) = atoi(arg);
    ...
    } else if (strcmp(argo, ARG_WAIT_MECHANISM) == 0 &&
               a->wait_mechanism == NULL) {
      int j;
      ...
      // if we haven't determined the mechanism, the user entered an invalid
      // mechanism value
...
void close_remaining_pipes(int** master_input_pipes, int** worker_input_pipes,
                           int num_workers) {
  int i;
...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < num_workers; i++) {
    ...
    if (cpid == 0) {
      close_fds(
          master_input_pipes[i][WRITE_END],
          worker_input_pipes[i][READ_END]); // close unused file descriptors
    ...
    close_fds(
        master_input_pipes[i][READ_END],
        worker_input_pipes[i][WRITE_END]); // close unused file descriptors
  }
  ...
  for (i = 1; i <= n; i++) {
    ...
    int write_result = write(master_input_pipes[worker_counter][WRITE_END], &t,
                             sizeof(int64_t));
  ...
  for (i = 0; i < num_workers; i++) {
    int64_t c = 0;
    int write_result =
        write(master_input_pipes[i][WRITE_END], &c, sizeof(int64_t));
    if (write_result != sizeof(int64_t)) {
